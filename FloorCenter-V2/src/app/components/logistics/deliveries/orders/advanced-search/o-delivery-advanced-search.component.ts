...
@Component({
	selector: "app-o-delivery-advanced-search",
	templateUrl : "./o-delivery-advanced-search.html"
})

export class OrdersDeliveryAdvancedSearchComponent {
	...
	private load(): void {
		...
		//get filtered delivery type pickup is not included
		this._commonViewService.getCommonList("deliverytypes",true)
              .subscribe(ddl => { this.deliveryModeList = ddl.filter(x => x["value"] != 1); }); 
		...
		else
		{
			...
			for(let i = 0; i < this.savedDeliveryStatus.length; i++) {
				switch(this.savedDeliveryStatus[i]) { 
				    ...
				    case "5": { 
				      	$("#chkDeliveryStatus3").prop("checked",true);
						break; 
   					} 
				}
			}
