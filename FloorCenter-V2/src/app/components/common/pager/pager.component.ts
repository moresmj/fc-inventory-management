...
export class PagerComponent implements OnInit{
    ...
    ngOnInit() {
        if (this.isDefaultFiltereredLoading) 
        { 
            this.filterPageWithParams(this.defaultFilter)
            this.defaultFilterNotYetLoaded = true;
        }
        else if (this.defaultNullLoading) {
            this.loadnNull()
        }
        else
        {
    ...
    filterList(record : any, searchKey : string) : boolean {
        for(let key in record)
        {
            if (record[key] != null) {
                ...
                if(typeof(record[key]) == "object" && key != 'importDetails' && key != 'storesHandled' && key != 'handled')
                {
    ...
    filterPageWithParams(params : RequestOptions) {
        this.spinnerService.show();
        this.moduleService = this._baseService.getService(this.module);
