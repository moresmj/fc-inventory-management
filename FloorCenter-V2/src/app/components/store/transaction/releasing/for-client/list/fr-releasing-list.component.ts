...
@Component({
	selector: 'app-fr-releasing-list',
	templateUrl: 'fr-releasing-list.html'
})



export class ForClientReleasingListComponent extends BaseComponent{

	public now: Date = new Date();

	module : string = "client-releasing";
	...
	onBtnUpdateClick(data : any){
		...
		this.updateForm = this.fb.group({
				...
				orNumber : new FormControl(data.orNumber,[Validators.required,Validators.maxLength(50)]),
				drNumber : new FormControl(data.drNumber,[Validators.required,Validators.maxLength(50)]),
   ...
   var options = {
       ...
       decimalseparator: '.',
       showLabels: true,
       headers: ['Transaction No.','PO No.', 'SI No.','OR No.','Release Date','Customer Name']   
	  };
	  
	  this._requestService.action = "transactions/releasing/forclientorder";
