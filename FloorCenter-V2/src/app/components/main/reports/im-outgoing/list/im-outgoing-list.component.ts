...
@Component({
	selector : 'app-im-outgoing-list',
	templateUrl : './im-outgoing-list.html'
})


export class InventoryMonitoringOutgoingListComponent extends BaseComponent {

	search : string;
	module : string = "outgoing-inventory";
	...
	toModel(detail : any): Inventories {
	    ...
	    let model = new Inventories({
	      ...
	      poNumber : detail.siNumber,
		  adjustment : detail.adjustment,
		  origin : detail.destination,
	...
	downloadList(){
		...
		var options = {
			...
			decimalseparator: '.',
			showLabels: true,
			headers: ['Item Code','Description','Tonality' ,'Size','Date', "DR No.", "SI No." , "Adjustment", "Origin", "Sales", "Interbranch", "Intercompany", "RTV"]				
		};

		this._requestService.action =  "inventories/main/inventory-monitoring/outgoing";
