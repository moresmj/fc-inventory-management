...
@Component({
  selector:  'app-register-breakage',
  templateUrl: './register-breakage.html'
})


export class RegisterBreakageComponent implements AfterViewInit, OnInit{
  ...
  itemList : any = [];
	...
	{
      this._apiBaseService.action = "imports/physicalcount/breakage/warehouse";
      this._requestService.action = "imports/physicalcount/breakage/warehouse";
        ...
        let detail = this.itemList.find(x => x.serialNumber == this.serialNumber);
            ...
            if (detail != undefined) {
              ...
              if (item === undefined) {
                ...
                let itemDetails = control.find(x => x.controls.ItemId.value == item.id);
                let newQuantity = parseInt(itemDetails.controls.Broken.value) + parseInt(qty);
                itemDetails.controls.Broken.setValue(newQuantity);
   ...
   /* addNewRow(data: any) {
    ...
    addNewRow(data: any,qty : any) {
      ...
      let newItem = this.fb.group({
        ItemId: new FormControl(data["id"], Validators.required),
        Broken: new FormControl(qty, [Validators.required, CustomValidator.quantity]),
        Remarks: ['']
      })
      // add new formgroup
      control.push(newItem);
