...
{
    public class STShowroomDeliveryDTOValidator : AbstractValidator<STShowroomDeliveryDTO>
    {

        private readonly DataContext _context;

        public STShowroomDeliveryDTOValidator(DataContext context)
        {
        ...
        private bool ValidItem(STShowroomDeliveryDTO model)
        {
            var item = this._context.STOrderDetails.Where(x => x.Id == model.STOrderDetailId && x.ItemId == model.ItemId).Count();
            if (item != 1)
            {
                return false;
            }
        ...
        private bool QuantityNotMoreThanApprovedQuantity(STShowroomDeliveryDTO model)
        {
            var item = this._context.STOrderDetails.Where(x => x.Id == model.STOrderDetailId && x.ItemId == model.ItemId).FirstOrDefault();
